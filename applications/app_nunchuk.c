...
//cdi
static volatile nunchuk_data_norm chuk_d_norm;
...
//cdi
void app_nunchuk_get_normalized_data(nunchuk_data_norm *chuk_data_norm)
{
	*chuk_data_norm = chuk_d_norm;
}

//cdi
void nunchuk_data_normalize(chuck_data *chuk_data)
{
	nunchuk_data_norm norm_value;

	norm_value.acc_x = ((float)chuk_data->acc_x - 500.0) / 500.0;
	norm_value.acc_y = ((float)chuk_data->acc_y - 500.0) / 500.0;
	norm_value.acc_z = ((float)chuk_data->acc_z - 500.0) / 500.0;
	norm_value.js_x = -((float)chuk_data->js_y - 128.0) / 128.0;	//cdi, nrf version
	norm_value.js_y = ((float)chuk_data->js_x - 128.0) / 128.0;		//cdi, nrf version
	norm_value.bt_c = chuk_data->bt_c;
	norm_value.bt_z = chuk_data->bt_z;

	chuk_d_norm = norm_value;
}

void app_nunchuk_update_output(chuck_data *data) {
	//cdi
	if(!APP_CUSTOM_TO_USE)
	{
		if (!output_running) {
			last_update_time = 0;
			output_running = true;
	...
	nunchuk_data_normalize(data);	//cdi
	timeout_reset();
}
