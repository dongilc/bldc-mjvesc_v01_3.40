...
void bldc_interface_init(void(*func)(unsigned char *data, unsigned int len)) {
	can_fwd_vesc = -1;
	send_func = func;
	// in our case, send_func -> send_packet_wrapper_spi_master -> send_packet_spi_master
}
...
void bldc_interface_send_packet(unsigned char *data, unsigned int len) {
	...
	else debug_printf("no selection of send_func\r\n");
}
...
void bldc_interface_process_packet(unsigned char *data, unsigned int len) {
	...
	case COMM_CUSTOM_APP_DATA:
		ind = 0;
		c_values.ch = data[ind++];
		c_values.fw_major = data[ind++];
		c_values.fw_minor = data[ind++];
		c_values.erpm = buffer_get_float32(data, 1.0, &ind);
...
//cdi
void bldc_interface_communicate_custom(uint8_t id, uint8_t COMM_SET, float set_value) {
	int32_t send_index = 0;
	fwd_can_append(send_buffer, &send_index);
	...
	switch(COMM_SET) {
		case COMM_SET_DUTY:
			buffer_append_float32(send_buffer, set_value, 100000.0, &send_index);
			break;
		case COMM_SET_CURRENT:
			buffer_append_float32(send_buffer, set_value, 1000.0, &send_index);
			break;
		case COMM_SET_CURRENT_BRAKE:
			buffer_append_float32(send_buffer, set_value, 1000.0, &send_index);
			break;
		case COMM_SET_RPM:
			buffer_append_int32(send_buffer, (int)set_value, &send_index);
			break;
		case COMM_SET_POS:
			buffer_append_float32(send_buffer, set_value, 1000000.0, &send_index);
			break;
		default:
			break;
	}
...
const char* bldc_interface_fault_to_string(mc_fault_code fault) {
	...
	case FAULT_CODE_DRV: return "FAULT_CODE_DRV8302";		//cdi
