...
void spi_lld_start(SPIDriver *spip) {
  ...
  spip->spi->CR1  = 0;
  if(spip->config->role == SPI_SLAVE)
  {

    spip->spi->CR1  = spip->config->cr1;// | SPI_CR1_RXONLY;//| SPI_CR1_SSM | SPI_CR1_SSI;
    spip->spi->CR2  = SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN;//SPI_CR2_SSOE | SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN;
