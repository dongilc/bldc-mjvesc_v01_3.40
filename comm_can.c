...
void comm_can_set_baud(CAN_BAUD baud) {
	...
	case CAN_BAUD_3M:	set_timing(1, 3, 1); break;	//cdi (brp, ts1, ts2), activate CAN_MANUAL_BAUDRATE@conf_general.h
	default: break;
	}
}
		...
		while (rx_frame_read != rx_frame_write) {
			...
			if (rxmsg.IDE == CAN_IDE_EXT) {
				...
				if (id == 255 || id == app_get_configuration()->controller_id) {
					...
					case CAN_PACKET_SET_ENCODER_RESET:
						// cdi
						encoder_data_reset();
						break;

					case CAN_PACKET_SET_DPS:
						// cdi
						ind = 0;
...
void comm_can_set_dps(uint8_t controller_id, float dps) {
	int32_t send_index = 0;
	uint8_t buffer[4];
...
//cdi
void comm_can_set_encoder_reset(uint8_t controller_id) {
	comm_can_transmit_eid(controller_id |
			((uint32_t)CAN_PACKET_SET_ENCODER_RESET << 8), 0, 0);
}
