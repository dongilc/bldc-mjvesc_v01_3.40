...
# setting.
# //cdi. added "$(CHIBIOS)/os/various/shell.c"
CSRC = $(STARTUPSRC) \
       $(KERNSRC) \
       $(PORTSRC) \
       $(OSALSRC) \
...
# Program for OSX _CDI
upload-osx: build/$(PROJECT).bin
		#qstlink2 --cli --erase --write build/$(PROJECT).bin
		#openocd -f interface/stlink-v2.cfg -c "set WORKAREASIZE 0x2000" -f target/stm32f4x_stlink.cfg -c "program build/$(PROJECT).elf verify reset" # Older openocd
		/Applications/GNU\ MCU\ Eclipse/OpenOCD/0.10.0-4-20171004-0812-dev/bin/openocd -f board/stm32f4discovery.cfg -c "reset_config trst_only combined" -c "program build/$(PROJECT).elf verify reset exit" # For openocd 0.9
